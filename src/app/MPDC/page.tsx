...
function MPDC() {
  ...
  const filteredData = parsedData
    ? parsedData.mpdc_entities.filter((item: MPDCEntity) => {
  ...
  const groupedData: { [key: string]: MPDCEntity[] } = filteredData.reduce(
    (acc, item) => {
    ...
    {} as { [key: string]: MPDCEntity[] }
  );

  const calculateAverages = (data: MPDCEntity[]) => {
    const sum = {
      NDVI: 0,
      NDWI: 0,
      GLI: 0,
