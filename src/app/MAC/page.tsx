...
function MAC() {
  ...
  const filteredData = parsedData
    ? parsedData.mac_entities.filter((item: MACEntity) => {
  ...
  // Group the filtered MAC data by Lat-Lon
  const groupedData: { [key: string]: MACEntity[] } = filteredData.reduce(
    (acc, item) => {
    ...
    {} as { [key: string]: MACEntity[] }
  );

  const calculateAverages = (data: MACEntity[]) => {
    const sum = {
      NDVI: 0,
      NDWI: 0,
      GLI: 0,
