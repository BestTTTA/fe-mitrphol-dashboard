...
function MPL() {
  ...
  const filteredData = parsedData
    ? parsedData.mpl_entities.filter((item: MPLEntity) => {
  ...
  const groupedData: { [key: string]: MPLEntity[] } = filteredData.reduce(
    (acc, item) => {
    ...
    {} as { [key: string]: MPLEntity[] }
  );

  const calculateAverages = (data: MPLEntity[]) => {
    const sum = {
      NDVI: 0,
      NDWI: 0,
      GLI: 0,
