...
function MKS() {
  ...
  const filteredData = parsedData
    ? parsedData.mks_entities.filter((item: MKSEntity) => {
  ...
  const groupedData: { [key: string]: MKSEntity[] } = filteredData.reduce(
    (acc, item) => {
    ...
    {} as { [key: string]: MKSEntity[] }
  );

  const calculateAverages = (data: MKSEntity[]) => {
    const sum = {
      NDVI: 0,
      NDWI: 0,
      GLI: 0,
